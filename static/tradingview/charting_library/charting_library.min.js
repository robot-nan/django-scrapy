(function (f) {
    ...
    if (!f.support.cors && f.ajaxTransport && window.XDomainRequest) {
        var h = /^https?:\/\//i, d = /^get|post$/i, a = new RegExp("^" + location.protocol, "i");
        f.ajaxTransport("* text html xml json", function (b, c, g) {
            if (b.crossDomain && b.async && d.test(b.type) && h.test(b.url) && a.test(b.url)) {
                var e = null;
                return {
                    send: function (a, d) {
                        var g = "", l = (c.dataType || "").toLowerCase();
                        e = new XDomainRequest;
                        /^\d+$/.test(c.timeout) && (e.timeout = c.timeout);
                        e.ontimeout = function () {
                            d(500, "timeout")
                        };
                        e.onload = function () {
                            var a = "Content-Length: " + e.responseText.length + "\r\nContent-Type: " + e.contentType, b = 200, c = "success", g = {text: e.responseText};
                            try {
                                if ("html" === l || /text\/html/i.test(e.contentType)) g.html = e.responseText; else if ("json" === l || "text" !== l && /\/json/i.test(e.contentType))try {
                                    g.json = f.parseJSON(e.responseText)
                                } catch (h) {
                                    b = 500, c = "parseerror"
                                } else if ("xml" === l || "text" !== l && /\/xml/i.test(e.contentType)) {
                                    var k = new ActiveXObject("Microsoft.XMLDOM");
                                    k.async = !1;
                                    try {
                                        k.loadXML(e.responseText)
                                    } catch (n) {
                                        k = void 0
                                    }
                                    if (!k || !k.documentElement || k.getElementsByTagName("parsererror").length)throw b = 500, c = "parseerror", "Invalid XML: " + e.responseText;
                                    g.xml = k
                                }
                            } catch (m) {
                                throw m;
                        ...
                        e.onerror = function () {
                            d(500, "error", {text: e.responseText})
                        };
...
(function () {
    ...
    if (!window.TradingView) {
        var h = {mobile: {disabledFeatures: "left_toolbar header_widget timeframes_toolbar edit_buttons_in_legend context_menus control_bar border_around_the_chart".split(" "), enabledFeatures: ["narrow_chart_enabled"]}}, d = {
            BARS: 0, CANDLES: 1, LINE: 2,
            AREA: 3, HEIKEN_ASHI: 8, HOLLOW_CANDLES: 9, version: function () {
                return "1.6 (internal id 1df239c3 @ 2016-05-26 08:17:21.737481)"
            }, gEl: function (a) {
                ...
                var b = {
                    width: 800, height: 500, symbol: "AA", interval: "D", timezone: "", container: "", path: "",
                    locale: "en", toolbar_bg: void 0, hideSymbolSearch: !1, hideSideToolbar: !1, enabledStudies: [], enabledDrawings: [], disabledDrawings: [], disabledStudies: [], drawingsAccess: void 0, studiesAccess: void 0, widgetbar: {datawindow: !1, details: !1, watchlist: !1, watchlist_settings: {default_symbols: []}}, overrides: {"mainSeriesProperties.showCountdown": !1}, studiesOverrides: {}, fullscreen: !1, autosize: !1, disabledFeatures: [], enabledFeatures: [], indicators_file_name: null, custom_css_url: null, auto_save_delay: null, debug: !1, time_frames: [{
                        text: "5y",
                        resolution: "W"
                    }, {text: "1y", resolution: "W"}, {text: "6m", resolution: "120"}, {text: "3m", resolution: "60"}, {text: "1m", resolution: "30"}, {text: "5d", resolution: "5"}, {text: "1d", resolution: "1"}], client_id: "0", user_id: "0", charts_storage_url: void 0, charts_storage_api_version: "1.0", logo: {}, favorites: {intervals: [], chartTypes: []}, rss_news_feed: null
                }, c = {
                    width: a.width, height: a.height, symbol: a.symbol, interval: a.interval, timezone: a.timezone, container: a.container_id, path: a.library_path, locale: a.locale, toolbar_bg: a.toolbar_bg,
                    ...
                    studyCountLimit: a.study_count_limit, symbolSearchRequestDelay: a.symbol_search_request_delay
                };
                f(c);
        ...
        d.widget.prototype = {
            _innerWindow: function () {
                ...
                function b() {
                    d.gEl(g.id).contentWindow.TradingView.GlobalEventsStorage.subscribe("chart_load_requested", a)
                }

                var c = this.render(), g = this, e;
                if (this.options.container) {
                    var f =
                        d.gEl(this.options.container);
                    f.innerHTML = c
                } else document.write(c);
                if (this.options.autosize || this.options.fullscreen) f = $(d.gEl(this.id)), f.css("width", "100%"), this.options.fullscreen || f.css("height", "100%");
                ...
                this.unsubscribeFromLoadRequestEvent = function () {
                    d.gEl(g.id).contentWindow.TradingView.GlobalEventsStorage.unsubscribe("chart_load_requested", a)
                };
                e = d.gEl(this.id);
                var h = null, h = function () {
                    d.unbindEvent(e, "load", h);
                    e.contentWindow.widgetReady(function () {
                        var a;
                        g._ready = !0;
                        for (a = g._ready_handlers.length; a--;)g._ready_handlers[a].call(g);
                        e.contentWindow._initializationFinished();
                        var c = d.gEl(g.id).contentWindow;
                        if (c.TradingView.GlobalEventsStorage) b(); else {
                            var f = null, f = function () {
                                b();
                                d.unbindEvent(c, "load", f)
                            };
                            d.bindEvent(c, "load", f)
                        }
                    })
                };
                d.bindEvent(e, "load", h)
            }, render: function () {
                window[this.options.uid] = {
                    datafeed: this.options.datafeed, tradingController: this.options.tradingController,
                ...
                a = a.createGroup({single: !0}).appendTo(c);
                ...
                this._innerWindow().setCallback("onIntervalChange",
                    a)
            }, onTick: function (a) {
                ...
                this._innerWindow().setCallback("onMarkClick",
                    a)
                ...
                this._innerWindow().createShortcutAction(a,
                    b)
                ...
                this.options = $.extend(!0, this.options, {overrides: a});
            ...
            showSampleOrderDialog: function (a) {
                ...
                this._innerWindow().showNoticeDialog(a)
            }, showConfirmDialog: function (a) {
                this._innerWindow().showConfirmDialog(a)
            }
        };
        "applyStudyTemplate setChartType clearMarks refreshMarks setVisibleRange createOrderLine createPositionLine createExecutionShape createVerticalLine createMultipointShape createShape removeEntity createStudy removeAllShapes removeAllStudies executeActionById executeAction".split(" ").forEach(function (a) {
            d.widget.prototype[a] = function () {
                console.warn("Method `" + a + "` is obsolete. Please use `chart()." + a + "` instead.");
                return this.activeChart()[a].apply(this.activeChart(), arguments)
            }
        });
        window.TradingView &&
